...
import hu.bme.mit.inf.symod.fa042.homework.chessclock.ChessClockStatemachine;
...
public class ReflectiveTimeInterfaceProvider {
	...
	public static Class<?> iTimer() {
		try {
			return Class.forName("hu.bme.mit.inf.symod.fa042.homework.ITimer");
	...
	public static Class<?> iTimerCallback() {
		if(isTimerAvailable()) {
			try {
				return Class.forName("hu.bme.mit.inf.symod.fa042.homework.ITimerCallback");
			} catch (ClassNotFoundException e) {
				//e.printStackTrace();
				return null;
			}
	...
	public static Object providePhysicsTimer() {
		if(isTimerAvailable()) {
			try {
				return Class.forName("hu.bme.mit.inf.symod.fa042.homework.TimerService").newInstance();
